            ...
            if (url === '/disconnect-sockets') {
                if (allowConnections) {
                    ...
                    while (sockets.length) {
                        ...
                        // NOTE: socket.end() half-closes the socket and
                        // socket.destroy() completely closes the connection.
                        // Just calling socket.destroy() will cause the socket
                        // to timeout and then eventually reconnect which might
                        // be closer to simulating a network outage, but is
                        // very slow compared to half-closing it with
                        // socket.end() because with socket.end() the client
                        // can immediately know about it and retry or respond
                        // however it wants.
                        socket.end();
                    }
                }
